...
\newcommand{\repo}{https://github.com/fragiovis/BigData-FlightAnalysis}
...
\begin{enumerate}
  ...
  \item \textbf{Selezione} di 9 colonne: \code{month}, \code{op\_unique\_carrier}, \code{origin},
        \code{dest}, \code{dep\_delay}, \code{arr\_delay}, \code{cancelled}, \code{cancellation\_code},
        \code{delay\_code}. Otto sono usate dai job; \code{dest} (aeroporto di arrivo) non serve alle due analisi,
        basate sull'aeroporto di partenza, ma è mantenuta perché la traccia consente di calcolare il Job~1 anche
        per tratta. Il file scende a \DS{mb}{flights_cleaned}\,MB.
\end{enumerate}

\begin{table}[H]
\centering\scriptsize
\input{generato/tab_qualita.tex}
...
\begin{table}[H]
\centering\small
\input{generato/tab_dataset.tex}
...
\input{generato/tab_righe_job_2.tex}
...
stesso dataset. Le tre tecnologie producono lo stesso risultato su tutti i dataset, sia in locale sia su YARN.
Durante lo
...
\begin{table}[H]
\centering\small
\input{generato/tab_tempi_local.tex}
...
\begin{table}[H]
\centering\small
\input{generato/tab_tempi_yarn.tex}
...
\begin{table}[H]
\centering\small
\input{generato/tab_tempi_aws.tex}
...
\begin{table}[H]
\centering\small
\input{generato/tab_ambienti.tex}
...
\begin{table}[H]
\centering\small
\input{generato/tab_metriche.tex}
...
\begin{itemize}
  ...
  \item \textbf{YARN}: lo stesso hardware diventa più lento (Tabella~\ref{tab:ambienti}): sul dataset completo da
        \RY{spark-core}{job_2} a \RY{spark-sql}{job_1} volte per Spark e fino a \RY{hive}{job_2} volte per il Job 2 di
        ...
        locale. Hive lo paga a ogni job MapReduce: il Job 2 resta tra \T{yarn}{hive}{job_2}{flights_1} e
...
\GU{local}{hive}{job_2}--\GU{local}{spark-core}{job_2} volte, su EMR di \GU{aws}{hive}{job_1}--\GU{aws}{spark-core}{job_2}
volte e su YARN di appena \GU{yarn}{hive}{job_2}--\GU{yarn}{spark-sql}{job_2} volte,
perché fino al dataset completo prevalgono i costi fissi. Oltre, la crescita si avvicina a quella dei dati: da 100\% a $5\times$ (input $\times 5$) in locale
...
(circa \GC{yarn}{hive}{job_1} volte, contro \GC{yarn}{spark-core}{job_1}--\GC{yarn}{spark-core}{job_2} di
